...
AND characters.charId=death.chrId;";
        ...
        public Account Register(string uuid, string password, bool isGuest)
        {
            ...
            cmd = CreateQuery();
            cmd.CommandText =
                "INSERT INTO accounts(uuid, password, name, guest, maxCharSlot, regTime) VALUES(@uuid, SHA1(@password), @name, @guest, 2, now());";
            cmd.Parameters.AddWithValue("@uuid", uuid);
            cmd.Parameters.AddWithValue("@password", password);
            ...
            if (ret)
            {
                ...
                cmd = CreateQuery();
                cmd.CommandText =
                    "INSERT INTO stats(accId, fame, totalFame, credits, totalCredits, souls, totalSouls) VALUES(@accId, 0, 0, 45, 45, 0, 0);";
